...
#
#svc=clint-ssh-router-eth0 ihost=clint.router.eth0 iport=80 to=tcp:eth0:80 $CURDIR/make-service.sh
svc=eth0-by-ip ihost=192.168.15.15 iport=80 to=tcp:eth0:80 $CURDIR/make-service.sh
svc=eth0-by-name ihost=eth0-by-name iport=80 to=tcp:eth0:80 $CURDIR/make-service.sh
...
for i in {14030..14040}; do ./loop.sh; $i done

iport=13000 svc=eth0.${iport}.ziti ihost=eth0.${iport}.ziti to=tcp:eth0:80 $CURDIR/make-service.sh
